...
void bcm63xx_xtm_dqmhandler(unsigned long channel);   /* forward reference */
...
int __init bcmxtmrt_init( void )
{
...
#if defined(CONFIG_BCM_FAP)
    /* Make xtmDqmHandler available to bcmPktDma lib */
    g_pXtmDqmHandler = bcm63xx_xtm_dqmhandler;   
#endif
    ...
    {
        g_GlobalInfo.rxIrq[i] = bcmPktDma_XtmSelectRxIrq(i);
    }
...
static int bcmxtmrt_xmit( pNBuff_t pNBuff, struct net_device *dev )
{
    ...
    if( pDevCtx->ulLinkState == LINK_UP )
    {
        ...
        /* XTM transmission is per Context, so pass pDevCtx as "channel" */
        bcmPktDma_XtmFreeXmitBufs((int)pDevCtx);
...
//printk("TX: pNBuff<0x%08x> pNBuffSkb<0x%08x> pData<0x%08x>\n", (int)pNBuff,(int)pNBuffSkb, (int)pData);
//DUMP_PKT(pData, 32);
//spin_lock_bh(&pGi->xtmlock_tx);
...
//spin_lock_bh(&pGi->xtmlock_tx);
                ...
                /* Xmit from a specific Tx Queue of a specific Context */
                /* Currently the 6362 is the only chip using the FAP for ETH/SAR rx/tx */
#if defined(CONFIG_BCM_FAP)
                bcmPktDma_XtmXmit((int)pTqi, pNBuff, len, HOST_VIA_DQM, isAtmCell, (int)pData, 0);
#else
                bcmPktDma_XtmXmit((int)pTqi, pNBuff, len, HOST_VIA_LINUX, isAtmCell, (int)pData, 0);
#endif
...
static void AssignRxBuffer(PRXBDINFO pRxBdInfo, UINT8 *pucData)
{
    ...
    bcmPktDma_XtmFreeRecvBuf((int)pRxBdInfo, (unsigned char *)pucData);
...
void FlushAssignRxBuffer(PRXBDINFO pRxBdInfo, UINT8 *pucData,
    UINT8 *pucEnd)
{
//printk("FlushAssignRxBuffer pRxBdInfo<0x%08x> pucData<0x%08x> pucEnd<0x%08x>\n", (int)pRxBdInfo, (int)pucData, (int)pucEnd );
    cache_flush_region(pucData, pucEnd);
    AssignRxBuffer(pRxBdInfo, pucData);
//printk("FlushAssignRxBuffer\n");
}
...
//printk("Recycling skb->data done\n");
    }
...
//printk("Recycle FKB pRxBdInfo<0x%08x> FKB<0x%08x> data<0x%08x>\n", (int)pRxBdInfo, (int)pFkb, (int)pucData);
    AssignRxBuffer(pRxBdInfo, pucData); /* No cache flush */
//printk("Recycle FKB done\n");
...
{
//printk("Recycle pNBuff<0x%08x> context<0x%08x>\n", (int)pNBuff, (int)context);
...
static irqreturn_t bcmxtmrt_rxisr(int nIrq, void *pRxDma)
{
    ...
    for( i = 0; i < MAX_DEV_CTXS; i++ )
    {
            ...
            pDevCtx->ulOpenState == XTMRT_DEV_OPENED )
        {
            ...
            channel = ((UINT32) pRxDma - (UINT32)pGi->pRxDmaBase) /
...
#if defined(CONFIG_BCM_FAP)
/* DQM handler for all XTM channels is the same */
/* Can be called from an IRQ directly */
void bcm63xx_xtm_dqmhandler(unsigned long channel)
{
    ...
    if( ulScheduled == 0 )
    {
        ...
        bcmPktDma_XtmClrRxIrq(channel);
        /* No Int Enable. DQM int is already enabled */
    }
...
static UINT32 bcmxtmrt_rxtask( UINT32 ulBudget, UINT32 *pulMoreToDo )
{
    ...
    //printk("bcmxtmrt_rxtask MAX_RECEIVE_QUEUES: %d\n", MAX_RECEIVE_QUEUES);
    ...
    do
    {
             ...
             i++, pRxBdInfo++ )
        {
            ...
            {
                //printk("ulRxPktProcessed: %d ulRxPktMax: %d\n", ulRxPktProcessed, ulRxPktMax);
                break;
            }
            else
            {
                                 ...
                                 FkBuff_t *pFkb, UINT16 bufStatus, int delLen )
{
   ...
   {       
       spin_lock_bh(&pGi->xtmlock_rx);
        ...
        spin_unlock_bh(&pGi->xtmlock_rx);
//printk("RX: skb<0x%08x> pBuf<0x%08x> len<%d>\n", (int)skb, (int)pBuf, (int)usLength);
    ...
    PXTMRT_TRANSMIT_QUEUE_ID pTxQId )
{
    ...
    if( (pTxQInfo->pMemBuf = kmalloc(ulSize, GFP_ATOMIC)) != NULL )
    {
            ...
            pTxQId->ulSubPriority < MAX_SUB_PRIORITIES )
        {
            ...
            /* save a ptr back to the defining Context for use in bcmPktDma */
            pTxQInfo->pdevctx = pDevCtx;
        }
        else
        {

...
vlan_dev_get_egress_qos_mask(struct net_device *dev, struct sk_buff *skb)
{
   ...
   /* egress priority map based on skb->vtag instead of skb->priority */
	mp = vlan_dev_info(dev)->egress_priority_map[(skb->vtag & 0xF)];
	while (mp) {
...
static int vlan_dev_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	 ...
	 */
#if defined(CONFIG_MIPS_BRCM) && defined(DOUBLE_TAGGING)

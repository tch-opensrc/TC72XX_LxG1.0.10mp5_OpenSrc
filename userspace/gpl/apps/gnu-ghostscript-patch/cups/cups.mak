...
CUPSCFLAGS=-fPIC -Os -g -fomit-frame-pointer -Wall -I$(includedir) -Wl,-L$(libdir)
CUPSLIBS=-lcupsimage -lcups -ldbus-1 -lssl -lcrypto -lz -lpthread -lm -lcrypt -ljpeg -lpng $(AVAHI_LIBS)
#CUPSLIBS=$(LIBS)
CUPSLIBDIRS=$(libdir)
CUPSSERVERBIN=$(libdir)/cups
CUPSSERVERROOT=$(prefix)/etc/cups
CUPSDATA=$(prefix)/share/cups
...
$(PDFTORASTER_XE): $(pdftoraster_)
	$(GLCC) $(CUPSLIBS) -DBINDIR='"$(bindir)"' -DGS='"$(GS)"' -o $@ $(pdftoraster_)

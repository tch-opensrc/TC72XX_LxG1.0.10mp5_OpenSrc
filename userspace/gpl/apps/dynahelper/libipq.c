                                    ...
                                    int timeout)
{
	...
	if (timeout != 0) {
		int ret;
		//BRCM Make it blocking.
		//struct timeval tv;
		fd_set read_fds;
		
//BRCM begin Make it blocking.
#if 0
		if (timeout < 0) {
...
//BRCM end
		...
		ret = select(h->fd+1, &read_fds, NULL, NULL, NULL);
		if (ret < 0) {
			if (errno == EINTR) {
				return 0;
			} else {
	...
	//BRCM add typecast to suppress warning
...
struct ipq_handle *ipq_create_handle(u_int32_t flags, u_int32_t protocol)
{
	...
	memset(h, 0, sizeof(struct ipq_handle));
        ...
        if (protocol == PF_INET)
                h->fd = socket(PF_NETLINK, SOCK_RAW, NETLINK_DYNAHELPER);

...
// nimah: This function pulls value selected in drop down and places it into according editable text box
function pullBoxData(dropdown, textbox) {
	
	document.getElementById(textbox).value = dropdown.options[dropdown.selectedIndex].value;
  ...
  showConfig: function (request) {
    ...
    $A(Config.tmpConfigXML.getElementsByTagName('section')).each(function (section) {
      ...
      body.className = 'navibox';
	   ...
	   // jayeshp: Remove ability to change location of config file
...
//          var parentSpan = Builder.node('span');
          values.each(function (val,i) {
			...
			// nimah: Create drop down (select box) that will sit below editable text box
			// nimah: Pass additional element (val) in order to pull previous value from text box into select box
			  div.appendChild(BuildElement.select(itemId+i,
                                  itemId,
                                  item.options,
                                  ConfigInitialValues.getValue(itemId) || item.default_value,
                                  item.short_description, '', val || ''));
...
//            }
            ...
            href = Builder.node('a',{style: 'position: relative; width: 6em; left: 50%;',href: 'javascript://'},'Remove ');
...
//        frag.appendChild(Builder.node('br'));
  ...
  _addItem: function(div) {
   ...
   // nimah: Modify cloned select boxes witn new id and also modify the onchange function passes to correspond to the right id 
   newSpan.getElementsByTagName('select')[0].id = id;
   newSpan.getElementsByTagName('select')[0].onchange = function(){pullBoxData(this, id);};
...
var BuildElement = {
  input: function(id,name,displayName,value,size,short_description,long_description) {
	...
	// nimah: If the name is general:mp3_dir (assigned to drop box and text box for mp3 dir selection) then modify style of input box
	// nimah: This is done to overlap select box and editable text box
    if (Config.isWritable) {
		if (name == 'general:mp3_dir')
		{
			input = Builder.node('input',{id: id,name: name,className: 'text',
                                             value: value,size: size, 
                                             style: 'position: absolute; top: 0px; left: 29%; z-index: 2; width: 242px;'});
		 }
		else 
		{								 
			input = Builder.node('input',{id: id,name: name,className: 'text',
    ...
    else 
    {
		if (name == 'general:mp3_dir')
		{
		  ...
		  input = Builder.node('input',{id: id,name: name,className: 'text',
												 ...
												 style: 'position: absolute; top: 29%; left: 0px; z-index: 2; width: 242px;'});
  ...
  select: function(id,displayName,options,value,short_description,long_description, val) {
    var frag = document.createDocumentFragment();
   
  
	// nimah: If the name is general:mp3_dir (assigned to drop box and text box for mp3 dir selection) then modify style of drop box
	// nimah: This is done to overlap select box and editable text box
    if (displayName == 'general:mp3_dir')
    {
		var select = Builder.node('select',{id: id,name: displayName,size: 1, 
					style: 'position: absolute; top: 0px; left: 29%; z-index: 1; width: 265px;',
					onchange: "pullBoxData(this, '"+id+"');"});
					
					
		
		
					
		// nimah: Pull current URL of site to create the location of dir.txt (in case IP or port is different)			
		var a = document.createElement("a");
		a.href = document.URL;
		var dURL = a.protocol + '//' + a.hostname + ':' + a.port + '/dir.txt';
	
	// nimah: Build a drop down option with previous value
	select.appendChild(Builder.node('option', {value: val}, val));
	// nimah: Download and parse dir.txt (which was created by lsmounts.cgi) then create according drop down option for each mount point detected
	var dirFile = new XMLHttpRequest();
	var dir;
	dirFile.open("GET", dURL, true);
	dirFile.onreadystatechange = function() 
	{
		if (dirFile.readyState === 4) 
		{  // Makes sure the document is ready to parse.
			if (dirFile.status === 200) 
			{  // Makes sure it's found the file.
				dir = dirFile.responseText.split("\n"); // Will separate each line into an array
				dir = dir.filter(String);
				for (i = 0; i < dir.length; i++)
				{
					if (dir[i] != " ")
					{
						select.appendChild(Builder.node('option', {value: dir[i]}, dir[i]));

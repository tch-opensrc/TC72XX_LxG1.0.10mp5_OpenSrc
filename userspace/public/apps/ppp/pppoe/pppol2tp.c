...
// brcm
int setdevname_pppol2tp(const char *cp);
...
// brcm
typedef struct  Ppp_L2tp_Connection_Struct {
    int pppol2tp_fd;
    char tunnel_name[64];
    char session_name[64];
    int pppol2tp_tunnel_id;
    int pppol2tp_session_id;
    struct channel * ppp_channel;
    bool auth;
    int status;
} Ppp_L2tp_Connection;

static Ppp_L2tp_Connection ppp_l2tp_connection;
...
//static int setdevname_pppol2tp(char **argv)
{
	...
	// brcm
	//printf("------ In setdevname_pppol2tp()\n");

	if (device_got_set)
		return 0;

	// brcm
	//if (!int_option(cp, &pppol2tp_fd))
	// brcm
	//printf("cp2 = %s\n", cp);

	// brcm
	sscanf(cp, "%d.%d.%d", &pppol2tp_fd, &pppol2tp_tunnel_id, &pppol2tp_session_id);
	ppp_l2tp_connection.pppol2tp_fd = pppol2tp_fd;
	ppp_l2tp_connection.pppol2tp_tunnel_id = pppol2tp_tunnel_id;
	ppp_l2tp_connection.pppol2tp_session_id = pppol2tp_session_id;
	//printf("Test: pppol2tp_fd=%d  pppol2tp_tunnel_id=%d pppol2tp_session_id=%d\n", 
	//		ppp_l2tp_connection.pppol2tp_fd, ppp_l2tp_connection.pppol2tp_tunnel_id, ppp_l2tp_connection.pppol2tp_session_id);
	//if (!int_option(*argv, &pppol2tp_fd))
	//	return 0;

	if(getsockname(ppp_l2tp_connection.pppol2tp_fd, (struct sockaddr *)&s, &len) < 0) {
	 ...
	 * feature available.
	 */
	if (getsockopt(ppp_l2tp_connection.pppol2tp_fd, SOL_PPPOL2TP, PPPOL2TP_SO_DEBUG,
		       &tmp, &tmp_len) < 0) {
	...
	// brcm
	ppp_l2tp_connection.ppp_channel = &pppol2tp_channel;
	the_channel = ppp_l2tp_connection.ppp_channel;
	device_got_set = 1;
...
{
	if(ppp_l2tp_connection.pppol2tp_fd == -1) {
		fatal("No PPPoL2TP FD specified");
	}

	return ppp_l2tp_connection.pppol2tp_fd;
}

static void disconnect_pppol2tp(void)
{
	if (ppp_l2tp_connection.pppol2tp_fd >= 0) {
		close(ppp_l2tp_connection.pppol2tp_fd);
		ppp_l2tp_connection.pppol2tp_fd = -1;
	}
}
				 ...
				 int accomp)
{
	...
	tid[0] = '\0';
	if (ppp_l2tp_connection.pppol2tp_tunnel_id > 0)
		sprintf(&tid[0], "%hu ", ppp_l2tp_connection.pppol2tp_tunnel_id);
	sid[0] = '\0';
	if (ppp_l2tp_connection.pppol2tp_session_id > 0)
		sprintf(&sid[0], "%hu ", ppp_l2tp_connection.pppol2tp_session_id);
	...
	dbglog("PPPoL2TP options: %s%s%s%s%s%s%s%s%sdebugmask %d",
	       ...
	       ppp_l2tp_connection.pppol2tp_tunnel_id ? "tid " : "", tid,
	       ppp_l2tp_connection.pppol2tp_session_id ? "sid " : "", sid,
	       pppol2tp_debug_mask);

	if (pppol2tp_recv_seq)
		if (setsockopt(ppp_l2tp_connection.pppol2tp_fd, SOL_PPPOL2TP, PPPOL2TP_SO_RECVSEQ,
			       &on, sizeof(on)) < 0)
			fatal("setsockopt(PPPOL2TP_RECVSEQ): %m");
	if (pppol2tp_send_seq)
		if (setsockopt(ppp_l2tp_connection.pppol2tp_fd, SOL_PPPOL2TP, PPPOL2TP_SO_SENDSEQ,
			       &on, sizeof(on)) < 0)
			fatal("setsockopt(PPPOL2TP_SENDSEQ): %m");
	if (pppol2tp_lns_mode)
		if (setsockopt(ppp_l2tp_connection.pppol2tp_fd, SOL_PPPOL2TP, PPPOL2TP_SO_LNSMODE,
			       &on, sizeof(on)) < 0)
			fatal("setsockopt(PPPOL2TP_LNSMODE): %m");
	if (pppol2tp_reorder_timeout)
		if (setsockopt(ppp_l2tp_connection.pppol2tp_fd, SOL_PPPOL2TP, PPPOL2TP_SO_REORDERTO,
			       &pppol2tp_reorder_timeout,
			       sizeof(pppol2tp_reorder_timeout)) < 0)
			fatal("setsockopt(PPPOL2TP_REORDERTO): %m");
	if (pppol2tp_debug_mask)
		if (setsockopt(ppp_l2tp_connection.pppol2tp_fd, SOL_PPPOL2TP, PPPOL2TP_SO_DEBUG,
			      ...
			      int accomp)
{
	...
	if ((ifunit >= 0) && ioctl(ppp_l2tp_connection.pppol2tp_fd, PPPIOCSMRU, (caddr_t) &mru) < 0)
		...
		if (opt == CI_ASYNCMAP && opt_len == 0x06) {
			...
			if (got_recv_accm && got_send_accm) {
				...
				if (pppol2tp_send_accm_hook != NULL) {
					(*pppol2tp_send_accm_hook)(ppp_l2tp_connection.pppol2tp_tunnel_id,
								   ppp_l2tp_connection.pppol2tp_session_id,
								   send_accm, recv_accm);
				}
				got_recv_accm = 0;
				got_send_accm = 0;
...
static void pppol2tp_ip_up_hook(void)
{
	...
	if (pppol2tp_ip_updown_hook != NULL) {
		(*pppol2tp_ip_updown_hook)(ppp_l2tp_connection.pppol2tp_tunnel_id,
					   ppp_l2tp_connection.pppol2tp_session_id, 1);
	}
}
...
static void pppol2tp_ip_down_hook(void)
{
	...
	if (pppol2tp_ip_updown_hook != NULL) {
		(*pppol2tp_ip_updown_hook)(ppp_l2tp_connection.pppol2tp_tunnel_id,
					   ppp_l2tp_connection.pppol2tp_session_id, 0);
	}
}
...
static void pppol2tp_check_options(void)
{
	...
	if (pppol2tp_lns_mode) {
		if ((ppp_l2tp_connection.pppol2tp_tunnel_id == 0) || (ppp_l2tp_connection.pppol2tp_session_id == 0)) {
